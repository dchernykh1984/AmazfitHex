...
export const SWAP_KEY = "swapRule";
...
// The pie rule, on or off. It is on by default because it is how Hex is normally
// played - without it the player who opens has a proven advantage - but it takes
// some explaining, and a player who would rather just keep the stone they put
// down can turn it off.
export const SWAP_OFF = 0;
export const SWAP_ON = 1;
export const SWAP_CHOICES = [SWAP_OFF, SWAP_ON];
export const DEFAULT_SWAP = SWAP_ON;

export function clampSwap(value) {
  return clampIndex(value, SWAP_CHOICES.length, DEFAULT_SWAP);
}

export function nextSwap(value) {
  return nextIndex(value, SWAP_CHOICES.length, DEFAULT_SWAP);
}

export function swapRuleEnabled(value) {
  return clampSwap(value) === SWAP_ON;
}

// The i18n key for the button that shows the setting's current state.
export function swapLabelKey(value) {
  return swapRuleEnabled(value) ? "swap_on" : "swap_off";
}
