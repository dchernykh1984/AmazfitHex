...
export function panLimits(layout, screenSize, padding) {
  ...
  for (let cell = 0; cell < layout.cellCount; cell++) {
    ...
    const needX = Math.ceil((needed * Math.abs(layout.offsetsX[cell])) / reach);
    const needY = Math.ceil((needed * Math.abs(layout.offsetsY[cell])) / reach);
    if (needX > limitX) {
      limitX = needX;
    }
    if (needY > limitY) {
...
export function isCellFullyVisible(layout, cell, panX, panY, screenSize, padding) {
...
export function isCellDrawable(layout, cell, panX, panY, screenSize) {
  return cellReach(layout, cell, panX, panY) <= screenSize / 2 + layout.scale;
}
...
export function panToCell(layout, cell, screenSize, padding) {
  const limits = panLimits(layout, screenSize, padding);
  return {
