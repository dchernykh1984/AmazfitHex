...
//
// Cells are laid out on the hex lattice and drawn as hexagons on a canvas. They
// used to be discs, one widget each, because the widget layer has no polygon to
// draw with; the canvas fills a hexagon out of horizontal boxes instead (see
// lib/layout/hex-shape.js), which costs one widget for the whole board and
// finally draws the shape the game is actually played on.
//
...
// distance from a hexagon's centre to a corner). Hexagons drawn at the full hex
// size tile edge to edge with no seam between them, so cells are drawn a little
// smaller: that leaves a thin gap which reads as a honeycomb rather than as one
// solid slab, and still fills far more of the board than the discs this
// replaced.
export const CELL_RATIO = 0.92;
...
export function hexLayout(screenSize, size, padding, minCap) {
  ...
  return {
    size: cells,
    cellCount,
    // The hex size the board came out at, and the circumradius each cell's
    // hexagon is drawn at - a little smaller, so cells do not share edges.
    scale,
    radius: cellRadius,
    centersX,
    centersY,
...
// How far a hexagon reaches towards its neighbour: the distance from its centre
// to the middle of a side, as opposed to `radius`, which reaches a corner. Two
// cells touch when their centres are twice this apart, so it is what says
// whether a board's cells overlap.
export function cellInradius(layout) {
  return (Math.sqrt(3) / 2) * layout.radius;
}
