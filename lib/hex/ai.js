...
//
//   * A stone on the opener's own edge. That edge is already theirs - it is one
//     of the two sides they are trying to join - so a stone standing on it has
//     bought almost nothing, and taking it over buys the same nothing. This is
//     what rules out the acute corners too, since those lie on both edges.
//   * A stone out towards a corner. What an opening is worth is how much of the
//     middle it commands, and centerBias is four times the hex distance from
//     the middle, so half a board's distance is the line drawn here.
//
...
export function shouldSwap(game) {
  ...
  const opener = game.cells[cell];
  const ownEdges = borderMask(opener, 0) | borderMask(opener, 1);
  if ((topology.edges[cell] & ownEdges) !== 0) {
    return false;
  }
