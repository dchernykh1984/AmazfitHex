...
// depth: an alpha-beta over a handful of candidate moves, deepened one ply at a
// time until a fixed budget of leaf evaluations runs out, keeping the best move
// of the last iteration that finished. Everything it touches is a flat integer
// buffer allocated once per board size and reused, so a whole move costs no
// allocations at all.
//
...
// down from the root so a win found sooner outranks the same win found later.
function negamax(context, cells, toMove, depth, ply, alpha, beta) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (connects(context, cells, toMove, move)) {
      ...
      value = -negamax(context, cells, foe, depth - 1, ply + 1, -beta, -bound);
    }
    cells[move] = EMPTY;
